...
resource "azurerm_monitor_metric_alert" "dynamic_alerts" {
  for_each = var.metric_alerts_dynamic
  name                = each.value.alert_name
  resource_group_name = azurerm_resource_group.monitoring.name
  scopes              = [each.value.resource_id]
  description         = each.value.description
  ...
  dynamic_criteria {
    ...
    failing_periods {
      minimum_failing_periods_to_trigger_alert = each.value.failing_periods_to_alert
      number_of_evaluation_periods              = each.value.evaluation_total_count
    }
  }
